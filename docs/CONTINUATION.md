...
> **Last updated:** 2026-06-13 (**Defect-B cont. — Torrents-CSV provider + YTS domain-rotation defect FIXED + EZTV resolved (skip); autonomous parallel-fleet loop running**). Operator: "continue" → "endless autonomous loop, 3-4 parallel subagents, real evidence, no bluff".
>
> **▶▶ CONTINUE — start here (resume in <5 min):** On-device `/providers` now exposes **9** (5 natives + TPB + YTS + Torrents-CSV + **BitSearch**). BitSearch added this loop (`internal/provider/curated/bitsearch/`, embed-hash `387c46d7`, both falsifiability mutations + live 2-query filter proof green). **NEXT free-text provider candidates (research-vetted, live-probed — `docs/research/2026-06-13-additional-curated-provider-candidates.md`):** (1) **Knaben** — `POST https://api.knaben.org/v1` with MINIMAL body `{"query":"<q>","size":<n>}` (the docs' `search_type:"score"` is an EZTV-class no-op — the live test MUST assert that to prevent reintroducing the bug); pre-built `magnetUrl`+`hash`. (2) **Nyaa** — `GET https://nyaa.si/?page=rss&q=<q>` (RSS/XML, anime niche; `<nyaa:infoHash>`, human size needs parsing). REJECTED w/ evidence: SolidTorrents(=BitSearch backend), 1337x(CF), TorrentGalaxy(DNS dead). Then: Phase-2 Cloudflare (FlareSolverr); api-app Crashlytics wiring; device-test + §6.AA distribute. **Subagent E (rutor/archiveorg/gutenberg/rutracker bluff-hunt) may still be running — re-verify + merge its worktree branch when it reports.**
>
