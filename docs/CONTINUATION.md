...
> - **⏭ REMAINING (critical path):** (1) **Go: `GET /v1/providers` endpoint** + openapi + **Jackett `ListIndexers` + `JackettIndexerProvider` + startup registration (collision guard)** + Go contract/e2e tests [stream 1 died at the rate-limit after only the interface]. (2) **Kotlin Phase 4: `ApiBackedTrackerClient` + `DynamicTrackerRegistry.populateFrom`** (core/tracker/client+registry) — unblocks the held stream-5/stream-3 work. (3) Integrate streams 3+5 + apiSelectionEnabled override + full `testDebugUnitTest` + `:app:assembleDebugAndroidTest`. (4) Device gate C39/C40 on Genymotion (Containers/§6.AH). (5) HelixQA scenario redo + run. (6) Distribute = SEPARATE operator-gated step (auth-rotation + §6.AA two-stage + §6.Z canary).
