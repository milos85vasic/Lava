...
> - **▶ CRITICAL GATE BLOCKER: LVA-008 (C11 nav-teardown) blocks BOTH C11 AND C06.** Core-flow device gate 2026-06-09 = **4/5 GREEN** (C00 cold-start, C01 launch+tracker, C07/C08 modal PASS with this session's new download-validator/credentials/api code) — only C06 (download) + C11 (archiveorg search) fail, both on the `search_input` teardown process-crash. NEXT: implement SA-2 **Candidate 2** — move `search_input`/`search_result` OUT of the inner nested `search` graph (`MobileNavigation.kt` `addSearch`) into the OUTER NavHost (host = Activity), eliminating the nested-host lifecycle-ordering bug; device-reverify C06 + C11; then §6.Y bump + §6.Z/§6.AA distribute. Evidence: `.lava-ci-evidence/genymotion/gate-coreflows-20260608T193752Z/`.
