...
>
> **▶▶ CONTINUE — start here (resume in <5 min):** All this-loop work is committed (see below). **Two parallel Go bluff-hunt subagents (worktree-isolated) were running at last checkpoint** — one on native tracker parsers, one on server/jackett glue; their worktree branches need re-verify + merge by the main stream when they report (do NOT blind-trust; re-run their falsifiability rehearsals before merging). **NEXT, in order:** (1) Phase-2 Cloudflare providers (1337x, TorrentGalaxy) via the FlareSolverr seam. (2) api-app Crashlytics wiring (still 404 for both variants — §6.AC/§6.O telemetry gap). (3) Harden TPB (apibay.org) + Torrents-CSV with the same **mirror-failover** pattern just added to YTS (single-domain rotation risk — see the YTS fix). (4) rebuild api-app + embed + device-test the EXACT artifact (§6.Z; device gate §6.AH-blocked on this macOS host) + §6.AA two-stage distribute. Design spec: `docs/superpowers/specs/2026-06-12-embedded-curated-providers-design.md`.
>
