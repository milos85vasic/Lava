...
> **Last updated:** 2026-06-13 (**Defect-B phase-1 cont. — 3rd embedded curated provider Torrents-CSV; EZTV surfaced as a §6.E honesty fork**). Operator: "continue".
>
> **▶▶ CONTINUE — start here (resume in <5 min):** Torrents-CSV curated Go provider is LANDED (`internal/provider/curated/torrentscsv/`, registered in `curated.RegisterAll` → BOTH startup paths, embed-hash `339d7828`). Full anti-bluff suite green: fixture parser + non-40-hex filter + empty-query/5xx + adapter-honesty tests; **both falsifiability mutations rehearsed** (seeders←leechers → `seeders=0, want 21`; drop hash filter → `got 3 results, want 2` + `"deadbeef" leaked`; reverted); **LIVE realtrackers PASS** incl. `TestLive_QueryActuallyFilters` (real `q=debian` genuinely filters → CapSearch honest); `go test ./...` 0 FAIL; `check-api-app-sync.sh` GREEN. On-device `/providers` now exposes 8 (5 natives + TPB + YTS + Torrents-CSV) once built. **NEXT, in order:** (1) **EZTV — OPERATOR DECISION REQUIRED (§6.E honesty fork):** the spec (line 96) assumed `?keywords=<q>` filters, but a LIVE probe PROVED it is a silent no-op — `keywords=ironman` and `keywords=batman` return the *identical* global latest-torrents list (count stays 1,055,207). Declaring `CapSearch` for EZTV would be a §6.E/§6.L bluff (search ignores the query → "search batman, get Deadliest Catch"), which is almost certainly why the prior agents "landed nothing" on it. Options: (a) skip EZTV; (b) ship EZTV honestly with NO free-text CapSearch (imdb-id-keyed only / browse-latest); (c) replace EZTV with another free-text JSON tracker. **Do not ship a query-ignoring EZTV.** (2) Phase-2 Cloudflare providers (1337x, TorrentGalaxy) via the FlareSolverr seam. (3) api-app Crashlytics wiring (still 404 for both variants — §6.AC/§6.O telemetry gap). (4) rebuild api-app + embed + device-test the EXACT artifact (§6.Z; device gate §6.AH-blocked on this macOS host) + §6.AA two-stage distribute. Design spec: `docs/superpowers/specs/2026-06-12-embedded-curated-providers-design.md`.
>
