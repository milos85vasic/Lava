...
> **Last updated:** 2026-06-12 (**🔎 CRASHLYTICS AUDIT + Defect-A fix: provider-catalogue fetch now crosses the self-signed-TLS + auth boundary**). Operator: "Check Crashlytics for both apps" + "only 4 providers in onboarding — investigate, fix, test".
...
> - **HONEST — Defect B still OPEN (do NOT redistribute as "many providers" yet):** even with Defect A fixed, the on-device api-app serves only its NATIVE providers until **embedded Jackett** lands (operator: "On device api app MUST have Jackett embedded same we have the API itself"). The client now correctly shows whatever the api-app serves; making that list large needs Defect B (Go Cardigann/Torznab indexer engine + bundled definitions in `lava-api-go`, embedded in api-app like the API itself). Owed next.
>
