...
> **Last updated:** 2026-05-31 (§6.L 69th — T7 fast-storage relocation + §6.AA Stage 1 debug DISTRIBUTED). Operator directive: "use T7 for storage-demanding work + update path references — T7 is our main fast storage." **§6.M storage relocation (verified):** the podman VM (`~/.local/share/containers`, 50 GB `.raw`) MOVED to `/Volumes/T7/containers` (symlink) + machine restarted; host data-volume freed ~50 GB (16 → 66 GiB). Already-on-T7 from a prior migration: `~/.android` (AVDs, 24 GB), `~/.cache`, `GRADLE_USER_HOME`. Map: `docs/ops/T7-fast-storage.md`. **Verified-green this cycle:** `assembleDebug` + `assembleRelease` (versionCode 1054, new pepper, aapt2-confirmed) + `assembleDebugAndroidTest` + lava-api-go binary (`--version`=2.3.23 build 2323) all built; `./gradlew testDebugUnitTest` BUILD SUCCESSFUL. **Auth rotation (operator-authorized "rotate autonomously"):** fresh pepper + `LAVA_AUTH_CURRENT_CLIENT_NAME=android-1.2.34-1054` + fresh UUID in gitignored `.env` (values never echoed). **§6.Z retest of the rebuilt NEW-PEPPER artifact:** C00 (cold-start canary) + C01 EXECUTED on Pixel_8/API35 via Containers runner (host-direct+HVF, T7-backed) → both `all_passed:true, gating:true, 0 failures` (attestations under `.lava-ci-evidence/2026-05-31-1.2.34-1054-challenge-matrix-repepper/`). **§6.AA Stage 1 (debug) DISTRIBUTED:** `firebase-distribute --debug-only` EXIT 0 → Firebase release `0f9a72d53suhg`; §6.P + Phase-1 Gate 4/5 (rotated pepper SHA `eef6729…` recorded) all passed; `last-version-debug`=1054. **STILL OWED (honest, NOT bluffed):** (1) **lava-api-go OCI image + compose boot** — `build_and_release.sh` image step STILL exits 125: the Dockerfile `COPY . .` copies the multi-GB `releases/` tree into the build container, refilling the 50 GB VM even after the T7 move. Fix = add `.containerignore` excluding `releases/` (+ `out/`, `build/`, `.git/`). The image is NOT needed for Firebase APK distribution; it IS needed for the "API running so we can test" item, which is therefore OWED. (2) **§6.AA Stage 2 (release)** — operator chose "debug verified → distribute release now"; the release-only distribute is the next action. Release APK ready at `releases/1.2.34/android-release/` (versionCode 1054). §6.L counter remains 69.
>
