...
> - **PENDING item 2 — Firebase: operator chose ONLY the 2 APK apps this cycle** (`:app` client + `:api-app`, debug+release each = 4 uploads); lava-api-go deferred (container-registry artifact, not a Firebase APK). §6.Z-gated on item 1's green gate. Stream D's `firebase-distribute.sh --app client|api-app` wiring design is ready; operator must run `firebase apps:create` for the 2 new `:api-app` Firebase apps (live-token prerequisite).
