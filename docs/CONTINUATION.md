...
> - **Build-stability fixes this close-out (LVA-076):** `core/apiengine` `process*JavaRes`→`buildCshared` task dependency (Gradle-validation BUILD FAILED on clean `.cxx`); Gradle heap 2g→4g (D8 dex-merge OOM). Known non-blocking: the api-go **container image** build (not a Firebase artifact) fails on podman-VM `/var/tmp` disk-full — a podman-machine maintenance item, does not affect the distributed APKs.
