...
> **Last updated:** 2026-06-02 (docs stream — on-device Lava API Phases C + D-infra documented). Doc-only: `docs/ON_DEVICE_API.md` extended with §4A (Phase C `:core:apiengine` JNI wrapper, commit `ef109760`) + §4B (Phase D-infra `:api-app` foreground service / `ApiEngineController` state machine / `NsdMdnsAdvertiser` / `ApiKeyStore`, commit `e62e0fa8`) + updated Mermaid diagrams; `docs/guides/ON_DEVICE_API_USER_GUIDE.md`, `README.md`, `AGENTS.md` updated to reflect the landed modules; §1 "On-device Lava API sub-project" subsection added below. D-ui / Phase E / sub-project-2 remain PENDING. No source files touched.
>
