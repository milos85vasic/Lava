...
>   1. **C02 test-fix commit** (`api-app/.../Challenge02ApiAppBootAndServeTest.kt` short-read gate-verdict + `OnDeviceApiClient.kt` `getWithKeyShortRead`) — in the working tree, UNCOMMITTED. Needs its §6.J Bluff-Audit clean rehearsal (bogus-key → assert(c) fast-401 fail → revert) which is BLOCKED by the S23 Ultra keyguard re-engaging. TOMORROW: unlock the S23 Ultra (R5CW33CBVQV) + keep awake (Settings→Dev options→Stay awake) → re-run the bogus-key rehearsal (full suite so C01 warms compose first) → confirm C02 fails at the assertNotEquals(401) → revert → commit the 2 files with the Bluff-Audit stamp → push.
...
>   3. **§6.AH-debt:** no-KVM/TCG containerized emulator for macOS (a stopped subagent left partial WIP in `submodules/containers` working tree, uncommitted — `cleanup.go`/`containerized.go` mods + `cleanup_os.go`; plus a built `emulator-matrix` binary that must NOT be committed). The §6.AG/§6.AH container/VM emulator path doesn't yet boot on macOS (host-direct adbd-offline wedge; the real S23 Ultra is the current §6.Z surface).
