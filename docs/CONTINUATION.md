...
> **Last updated:** 2026-06-02 (FRESH SESSION — item 2 PARTIAL: **api-app 0.1.0-1 debug SHIPPED to Firebase** (release `0aksve4ung948`, app `digital.vasic.lava.api.dev`); **client 1.2.36-1056 distribute DEFERRED — §6.Z gate BLOCKED by a host emulator-boot wedge**. Wiring (Stream A), `firebase apps:create` ×2, §6.Y bump, pepper rotation, CHANGELOG + snapshots all DONE. Streams B (§11.4.107 draft) + C (docs_chain plan) held pending operator inputs.).
>
> **▶ NEXT FRESH SESSION — RESUME HERE:** **item 2 is two-thirds done.**
> - ✅ Firebase `--app client|api-app` wiring (commit `f810d521`); ✅ `firebase apps:create` ×2 (api-app IDs in `.env`: `LAVA_FIREBASE_API_APP_ID`/`LAVA_FIREBASE_API_APP_DEV_APP_ID`); ✅ §6.Y client bump 1.2.36-1056 + pepper rotated + client-name `android-1.2.36-1056`; ✅ CHANGELOG + snapshots both apps; ✅ all 4 APKs built (`releases/1.2.36/` + `releases/api-app/0.1.0/`); ✅ both gate runners gained `--boot-timeout` passthrough.
