...
>   1. **Device gate (§6.G/§6.J load-bearing):** run C39 (dynamic discovery) + C40 (jackett indexer provider) on the Genymotion VM via `scripts/run-genymotion-challenges.sh` with `-DlavaDynamicDiscoveryGate` (+ `-DlavaDynamicProviderName`/`-DlavaJackettIndexerName`) AND a running **api-app rebuilt with the new `/providers` endpoint** serving a real catalogue (Jackett configured for C40). They `assumeTrue(gateEnabled())`-skip without the gate host — so this needs: rebuild api-app (versions already bumped 0.2.3-7) + boot it on the VM + point the client's chosen API at it.
