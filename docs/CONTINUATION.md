...
> - **OWED:** (1) the C02 test-fix commit (`Challenge02ApiAppBootAndServeTest.kt` + `OnDeviceApiClient.kt`) needs its §6.J Bluff-Audit clean rehearsal (bogus-key → assert(c) 401 fail) — BLOCKED because the S23 Ultra keyguard re-engaged mid-session; complete once the device is unlocked + kept awake. (2) §6.AH-debt: no-KVM containerized emulator for macOS (stopped subagent left partial work in `submodules/containers` working tree, uncommitted). (3) §6.AH client gate: full C00/C01 on an API≤35 surface once the container emulator works.).
>
