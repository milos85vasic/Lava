...
> - **Lava glue** (`83b436e3`): `scripts/run-genymotion-challenges.sh` (thin glue → Containers CLI → Gradle connectedDebugAndroidTest vs the VM serial; §6.AG/§6.AH); guard hook recognizes Genymotion VM serials via `LAVA_REAL_DEVICE_SERIALS`. **Challenge00 device-integration PROVEN** (serial+identity probed live); full connected-test PASS **PENDING_QUIET_RERUN** — first attempt hit KSP corruption from 15 concurrent gradle daemons (the parallel P4 subagents); recorded honestly per §11.4.6, NOT bluffed. **LESSON: full-app connectedDebugAndroidTest MUST NOT run concurrent with other gradle in the same checkout.**
