...
**Last updated:** 2026-05-16 (HelixQA Option 1 wiring landed — `CM-HELIXQA-WIRING` gate added)
...
| `CM-HELIXQA-WIRING` | HelixConstitution §11.4.27 (CONST-050) full test-type coverage including HelixQA + Lava §6.AE Comprehensive Challenge Coverage | `scripts/run-helixqa-challenges.sh` (Lava-side wrapper invoking the 11 HelixQA Challenge scripts at `Submodules/HelixQA/challenges/scripts/`; captures per-script stdout+stderr to log files; emits roll-up attestation JSON; classifies exit codes 0/2/other → PASS/SKIP/FAIL; refuses to silently mask missing submodule or wiring drift — exit 3 with actionable remediation per §6.J anti-bluff posture). Wired into `scripts/run-challenge-matrix.sh` via `--include-helixqa` opt-in flag (OFF by default so existing matrix runs are unaffected). Hermetic test: `tests/check-constitution/test_helixqa_wiring.sh` (4 fixtures: all-green path + script-missing + submodule-absent + FAIL-classification-not-mis-categorized-as-SKIP). Docs: `docs/scripts/run-helixqa-challenges.sh.md` + `docs/scripts/run-challenge-matrix.sh.md`. Design: `docs/plans/2026-05-16-helixqa-integration-design.md` Option 1 — shell-level wiring. PARTIAL: HelixQA scripts currently run on HOST not inside Containers submodule per §6.X (open question #1; owed in future cycle). Many HelixQA scripts are Linux-only (systemctl/journalctl); macOS hosts will see FAIL/SKIP outcomes that are HONEST acknowledgments of the wrong-OS gap rather than false-pass. | ✅ wired (Option 1 — shell wiring); PARTIAL pending §6.X container-wrap follow-up |
