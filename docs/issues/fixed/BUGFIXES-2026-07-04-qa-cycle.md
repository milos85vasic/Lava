...
"Search does not work, never get any results" — likely the 502-egress provider set (archiveorg/yts/thepiratebay/tokyotosho/nyaa/torrentscsv/bitsearch/knaben) when the client is pointed at the goapi on a VPN-egress host. rutracker/kinozal/gutenberg work. The fix is per-destination egress routing (operator-infra decision per CONTINUATION 2026-07-02), NOT a parser patch and NOT a global bypass (a global bypass breaks rutracker/kinozal which need the VPN exit-IP).
