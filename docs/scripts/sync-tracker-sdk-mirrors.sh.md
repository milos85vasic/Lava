# `scripts/sync-tracker-sdk-mirrors.sh` — User Guide
...
Stub doc generated from the script's in-source header comment. See `scripts/sync-tracker-sdk-mirrors.sh` for canonical behavior. This stub exists so the `CM-SCRIPT-DOCS-SYNC` pre-push gate (`.githooks/pre-push` Check 9) starts gating modifications to this script.
...
- `scripts/sync-tracker-sdk-mirrors.sh` — the script itself
