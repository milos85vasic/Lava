...
Run this against a disposable clone, never against the real `master` on first use (per the plan's Human Checkpoint #2). **T054 (the mandatory review gating this) was accepted 2026-09-18** — see the corrected note under Step B below — but the script has still never been invoked against a real submodule upstream **from this scenario**: independent verification during the T054 review ran only the four hermetic test suites (`tests/pipeline/test_advance_all_submodules*.sh`, 634/634 assertions passing) against disposable git fixtures, exactly as this paragraph originally described. Step A itself remains to be exercised for real, against a disposable clone, as the text below prescribes.
...
So the pass condition of this scenario is a **committed, pushed, clean** repository — never a staged one. `scripts/pipeline/phase-07-closure.sh` is the phase that produces it, and **it does not exist yet as of this writing**: T054 gated T055 and has now cleared.

**T054 status, corrected 2026-09-18 (this paragraph previously said "four completed rounds, every verdict APPROVE-WITH-FIXES, never a clean approval" — that was stale the moment it was written and is replaced here rather than left to mislead the next reader).** The review continued through a fifth adversarial round after the fourth, and the round count that actually ran was **3 → 5 → 7 → 9 → 10 → 11 → 12** distinct fixture-proven escape routes, all found inside R-005 step 6 (committing and publishing a submodule's own uncommitted work). Guarding provably stopped converging — the eleventh escape smuggled 4000 bytes through a commit object's own author-name header past all five prior publish guards, and a convergent byte-equality instrument built to catch exactly that was itself defeated (`git commit-tree` inherits `i18n.commitEncoding`, so the expected commit was built in the same adversary-controlled config context and the poison cancelled out on both sides). The resolution landed in commit `97cc64a8` (2026-08-27): **step 6 and `--publish-local-modifications` were removed outright**, not re-guarded a sixth time. The script now issues no `git push` and creates no commit on any path — a structural guarantee rather than a flag-selection one. Independently re-verified 2026-09-18 during the closing T054 review: the file read line-by-line, all four hermetic suites re-run from scratch (74 + 61 + 43 + 456 = 634 assertions, 0 failures), and a grep confirmed zero reachable `git push`/`git commit` outside an isolated self-test subshell. **One risk remains open and disclosed, not closed by this review**: the step-5 rebuild-and-test command inherits this host's live push credentials (SSH agent, `gh`/`glab` config) with no sandbox, so a supply-chain-compromised build script in an advanced submodule could push using those ambient credentials — bounded today because `GOVERNANCE_ALLOW` authorizes only `helixqa`, but not eliminated. Tracked as owed work (a `bwrap` sandbox around the verify command), not a blocker for T055/T056/T062's scope. Stating the assertions here without being able to run them is deliberate — this is what Step B will assert on the day it runs, not a claim that it has:
