...
| `[repo-path]` | Positional. Names **the repository the run operates on, entirely**: the phases inspect it, its `HEAD` is the report's `commit_sha`, and the run's evidence directory is created inside it. The orchestrator canonicalises the path and `chdir`s to it before creating anything, so every relative `.lava-ci-evidence/…` path — the orchestrator's, `lib/run-report.sh`'s, and each phase script's — resolves under that one repository. Defaults to the repository containing the current working directory. |

**CORRECTED 2026-08-26**: the `[repo-path]` row above previously read "Controls which repository the phases **inspect**; does **not** relocate the run report, which is always written relative to the current working directory." That was not a design decision — it was a P0 defect with a contract row around it. Measured: `pipeline-build-test-distribute.sh /tmp/fixture --until precondition`, run from this repository against a *different* clean fixture on `master`, wrote a report into **this** repository's `.lava-ci-evidence/pipeline-runs/` carrying `"commit_sha": "<this repo's HEAD>"` and `"outcome": "PASS"` — a verdict about the fixture, filed under a commit of this repository that nothing had tested. §6.Z, §6.AK and §6.AA clause 8 all match evidence to artifacts *by commit SHA*, so that artifact sat exactly where a gate reads it as proof. Three were produced before the cause was found. The split base is closed by the `chdir` described above; regression coverage is `tests/pipeline/test_wrong_repo_attribution.sh`, which drives the orchestrator with its CWD and its `[repo-path]` deliberately disagreeing — the gap every prior suite kept accidentally closed by `cd`-ing into the fixture *and* passing it as the override.
