...
|---|---|
| `submodules/tracker_sdk/api` | Generic feature-bearing primitives (`MirrorUrl`, `TrackerDescriptor`-shape, `Mirror` health states). Lava-agnostic. |
...
See parent Lava `/CLAUDE.md` §6.AG. Every test needing an Android device (Compose UI Challenge Tests, `connectedAndroidTest`, `-PdeviceTests=true`, the §6.AE matrix, any emulator-bearing gate) MUST obtain it from an emulator instance brought up + managed by the `vasic-digital/Containers` submodule (`submodules/containers/pkg/emulator/` + `cmd/emulator-matrix`), via `scripts/run-challenge-matrix.sh` / `scripts/run-emulator-tests.sh`. The device is a **cold-booted emulator AVD, NEVER a live/physical ADB device** (physical devices are presumed in use by other projects and MUST NOT be targeted). Per-OS implementation (the §6.X acceleration model): Linux x86_64 → emulator INSIDE a podman/docker container (`--device /dev/kvm`); macOS/arm64 → host-direct+HVF cold-boot (a Linux container cannot reach `/dev/kvm` — absent in the podman VM — nor HVF, a host-only API; this is the §6.X-resolved macOS gate path, still Containers-`emulator-matrix`-driven, still an emulator, NOT a live device); Windows → host-direct+WHPX. No live-device fallback — a test redirected to a physical `adb` device because an emulator was slow/unavailable is a §6.AG violation; the test is OWED, not silently redirected (§6.Z/§6.J). Project-level clause per operator instruction + §11.4.35 (consumer-side, NOT the shared `constitution/` submodule). `Classification:` project-specific.

## §6.AM — Multi-Track Development System Adoption (added 2026-08-14)

See parent Lava `/CLAUDE.md` §6.AM for the full clause. Per operator directive (2026-08-14): "current project directory as Track 1 (main track)... This MUST BE default and only way of working... from now on forevermore!" Lava adopts the HelixConstitution multi-track engine (`constitution/scripts/multitrack/`, §11.4.176/§11.4.178/§11.4.179/§11.4.187/§11.4.191/§11.4.192/§11.4.196/§11.4.198) as the default and only way of working. Per-host config: `config/multitrack/nezha.yaml` — Track 1 = this checkout (`/run/media/milosvasic/DATA4TB/Projects/lava`, drive serial `511240401212000405`, btrfs, branch `master`); the system drive (serial `S4EWNS0X108183F`) is `protected_drives`-listed and never a track target. Bootstrap via `constitution/scripts/multitrack/multitrack_bootstrap.sh "$(pwd)"` (idempotent, re-run on topology change). Track 1's `mt_plan()` status reads `NON-EMPTY` (informational only, not fatal) because it is an already-in-use checkout being registered, not a fresh drive being provisioned. Additional tracks (operator: "we will be enabling and adding more tracks in near future") require a new drive or a §11.4.179 reflink-clone-with-own-`.git` — never a shared-common-dir `git worktree`. Does not relax any existing Anti-Bluff Pact / Local-Only CI/CD / §6.W / Host-Stability rule. `Classification:` project-specific (topology data); the engine itself is universal per HelixConstitution.
...
When an artifact has both a debug and a release variant, distribute MUST happen in TWO STAGES with operator-confirmed verification between them. Stage 1: `firebase-distribute.sh --debug-only` distributes the debug APK to the `.dev`-suffixed app ID; operator (or designated tester) verifies the **Firebase-distributed** debug APK on the failure-surface device class. Stage 2: ONLY AFTER written debug verification, `firebase-distribute.sh --release-only` distributes the release APK; §6.Z evidence file appended with release-stage section. There is no combined distribute: `--debug-and-release` / `--both` are RETIRED as of 2026-08-26 (LVA-120) and now exit 1; `MODE` has exactly two reachable values, `debug` (the flagless default) and `release`. The R8 / minification surprise class (§6.Z forensic anchor was a release-only `painterResource()` rejection) is the load-bearing reason: staging surfaces non-R8 bugs at debug's smaller blast radius AND isolates R8-specific failures to the release stage. §6.AA-debt is CLOSED (see `CLAUDE.md` §6.AA-debt): the `--debug-only` default, the refusal of an out-of-order `--release-only`, and the paired `last-version-{debug,release}` per-channel pointers are all mechanically enforced in `scripts/firebase-distribute.sh`. §6.AA clause 8 (Pipeline Distribution Path) lets a qualifying Pipeline Run Report stand in for the written operator confirmation BETWEEN the two stages, for `scripts/pipeline/phase-05-distribute.sh` only, under eight conjunctive conditions lettered (A)–(H); `CLAUDE.md` §6.AA clause 8 is authoritative for that condition set and is deliberately not restated here. Forensic anchor: 2026-05-14 operator: "for purposes like this one we shall distribute via Firebase DEV / DEBUG version only. Once we try it, you continue and once all verified you distribute RELEASE too!"
