...
/** Thrown when [NnmclubHttpClient] short-circuits a request because the breaker is OPEN. */
            ...
            if (state == State.OPEN) {
                ...
                if (elapsed >= resetTimeoutMillis) {
                    ...
                    throw CircuitBreakerOpenException(
                        "NNM-Club circuit breaker OPEN; ${resetTimeoutMillis - elapsed}ms until probe.",
                    )
                }
            }
        }
...
/**
 * Authenticated HTTP transport for the NNM-Club plugin.
 *
...
class NnmclubHttpClient internal constructor(
        ...
        breaker.guard {
            concurrency.withPermit {
                executeOrThrow(
                    ...
                    require(it.isSuccessful) { "NNM-Club download failed: HTTP ${it.code} for $url" }
    ...
    private fun executeOrThrow(request: Request): Response {
        ...
        if (response.code in 500..599) {
            response.close()
            throw IOException("NNM-Club mirror returned HTTP ${response.code} for ${request.url}")
        }
        return response
    }
    ...
    fun cookieValue(name: String): String? {
        val nnmUrl = HttpUrl.Builder().scheme("https").host("nnmclub.to").build()
        cookieJar.loadForRequest(nnmUrl).firstOrNull { it.name == name }?.let { return it.value }
