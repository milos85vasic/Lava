...
// §11.4.85 STRESS + CHAOS coverage for the bounded-retry-INSIDE-the-breaker logic
// on the nnmclub.to client (maxAttempts=3, 500ms backoff, transient =
// network/timeout-or-5xx, terminal never retried — see doWithRetry / Fetch in
// client.go). This is the circuit-breaker class: a recovered transient must NOT
// move the breaker toward OPEN (GetFailures stays 0, state stays Closed), because
// doWithRetry returns nil on eventual success so breaker.Execute sees a success.
//
// Anti-Bluff (§6.J): the SUT is the REAL nnmclub.Client (NewClient) hitting a
// REAL httptest.Server. NO live calls. Primary assertions are on the USER-VISIBLE
// outcome (recovered HTTP 200 body / a clean error) AND the user-affecting
// breaker state (Closed ⇒ nnmclub stays usable; Open ⇒ user is locked out for
// 10s). Secondary on counts/latency.
//
// FALSIFIABILITY REHEARSAL (§6.J clause 2): break the retry by making the
// http.Do-error and 5xx branches in Fetch return `false` (terminal) instead of
// `true` (transient). TestStress_ConcurrentFlakyUpstream and
...
// the breaker eventually trips OPEN. Revert → pass.
...
// With the in-breaker retry (3 attempts), a caller drawing one 503 retries onto a
// 200 and recovers WITHOUT the breaker counting a failure.
func flakyUpstream(t *testing.T, failEvery int64) (*httptest.Server, *int64, *int64) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if failEvery > 0 && n%failEvery == 0 {
			...
			http.Error(w, "nnmclub flaky", http.StatusServiceUnavailable)
			return
		}
...
func TestStress_ConcurrentFlakyUpstream(t *testing.T) {
	...
	// One shared client (matches production: one breaker fields all nnmclub
	...
	for i := 0; i < concurrency; i++ {
		...
		go func() {
			...
			<-start
			t0 := time.Now()
			body, status, err := c.Fetch(ctx, "/forum/tracker.php", "")
	...
	// Closed (nnmclub stays usable) with 0 consecutive failures.
	...
	t.Logf("STRESS nnmclub: concurrency=%d ok=%d upstream_hits=%d upstream_503s=%d breaker=%s/%d p50=%s p95=%s goroutines before=%d after=%d",
...
// recovers to HTTP 200 AND each call's breaker stays Closed with 0 failures. A
// separate client+breaker per call isolates the breaker invariant per call.
func TestChaos_TransientKeepsBreakerClosed(t *testing.T) {
	...
	t.Logf("CHAOS nnmclub transient: %d/%d calls recovered with breaker Closed and 0 failures", okCount, concurrency)
}
...
// breaker counts exactly ONE failure (MaxFailures=5, so it stays Closed — not yet
// tripped). Asserts the call returns a non-nil error within a bounded time.
func TestChaos_PersistentFailsCleanly(t *testing.T) {
	...
	t0 := time.Now()
	_, status, err := c.Fetch(ctx, "/forum/tracker.php", "")
	elapsed := time.Since(t0)

	if err == nil {
		t.Fatalf("persistent 503 returned nil error (status=%d)", status)
	}
	// 3 attempts with 2x500ms backoff ~= 1s; must NOT hang to the ctx deadline or
	// the client's 30s HTTP timeout.
	...
	t.Logf("CHAOS nnmclub persistent: clean failure in %s after %d attempts (breaker=%s/%d err=%v)",
...
// trips the breaker OPEN (after MaxFailures=5 genuine post-retry failures), which
// is the user-visible "nnmclub temporarily unavailable" outcome. Once OPEN,
// further calls fail fast without an upstream hit — the breaker protecting the
// user from hammering a dead upstream. Asserts the transition and the fast-fail.
func TestChaos_PersistentTripsBreakerOpen(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		_, _, err := c.Fetch(ctx, "/forum/tracker.php", "")
		if err == nil {
	...
	// User-visible OPEN-breaker contract: the next call fails FAST, with a non-nil
	// error, WITHOUT contacting the (dead) upstream — protecting the user from
	// hammering it.
	...
	t.Logf("CHAOS nnmclub breaker-trip: OPEN after 5 failures, then fast-fails in %s with no upstream hit (err=%v)", elapsed, err)
}
