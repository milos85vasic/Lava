...
// transient timeout/5xx counted as a breaker failure; 5 of them (MaxFailures)
// OPEN the breaker for ~10s and lock the user out of rutracker. The in-closure
// retry converts the transient into a success, so breaker.GetFailures() stays 0
// and the state stays Closed (a successful Execute resets the consecutive-
// failure counter).
//
...
func TestRetriesOnTransient5xx(t *testing.T) {
	...
	if string(body) != "<html>recovered</html>" {
		t.Fatalf("body=%q want the post-retry 200 body", string(body))
	}
