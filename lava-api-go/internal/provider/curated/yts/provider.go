...
const canonicalSite = "https://yts.mx"
...
func (a *ProviderAdapter) DisplayName() string { return "YTS" }

// AuthType returns NONE — YTS search is anonymous.
...
// Capabilities: YTS yields search rows with magnet links (no .torrent files in
// this flow), so this provider honestly declares SEARCH + MAGNET_LINK only (§6.E).
...
// Browse is unsupported (YTS list_movies is search-only in this flow).
...
// GetTorrent is unsupported — this flow serves magnet links, not .torrent files.
...
// HealthCheck probes YTS with a fixed query.
