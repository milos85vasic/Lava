...
// bitsearch curated JSON-API provider (maxAttempts=3, 500ms backoff, transient =
// network/timeout-or-5xx, terminal never retried — see fetchResults /
// fetchResultsOnce in client.go).
//
...
// c.fetchResults call in Search with a single c.fetchResultsOnce call (drop the
// retry loop). TestStress_ConcurrentFlakyServer and TestChaos_TransientRecovers
// then FAIL: the flaky server's first-hit 503 is surfaced as ErrUnknown instead
// of retried, so "all N concurrent Search() calls eventually succeed" reports the
// 503 errors. Revert → pass.

// stressBody is an /api/v1/search response carrying exactly one valid 40-hex hash
// so a 200 yields exactly one downloadable SearchItem (user-visible row).
const stressBody = `{"results":[{"infohash":"8e1e7ad6a7198d1bea2d8564f40ec3480c490301","title":"Stress Sample 1080p","size":732912680,"category":1,"seeders":100,"leechers":7,"updatedAt":"2026-06-16T00:00:00Z"}]}`
...
func TestStress_ConcurrentFlakyServer(t *testing.T) {
	...
	for i := 0; i < concurrency; i++ {
		...
		go func() {
			...
			<-start
			t0 := time.Now()
			res, err := client.Search(ctx, "stress", 1)
	...
	t.Logf("STRESS bitsearch: concurrency=%d ok=%d server_hits=%d server_503s=%d p50=%s p95=%s goroutines before=%d after=%d",
...
func TestStress_LatencyPercentiles(t *testing.T) {
	...
	t.Logf("STRESS bitsearch latency: iters=%d p50=%s p95=%s", iters, percentile(lats, 0.50), p95)
}
...
func TestChaos_TransientRecovers(t *testing.T) {
	...
	for i := 0; i < concurrency; i++ {
		...
		go func() {
			...
			res, err := NewClient(srv.URL).Search(ctx, "chaos", 1)
			if err != nil {
	...
	t.Logf("CHAOS bitsearch transient: %d/%d calls recovered from a first-hit 503", okCount, concurrency)
}
...
func TestChaos_PersistentFailsCleanly(t *testing.T) {
	...
	t0 := time.Now()
	res, err := NewClient(srv.URL).Search(ctx, "persistent", 1)
	elapsed := time.Since(t0)
	...
	t.Logf("CHAOS bitsearch persistent: clean failure in %s after %d attempts (err=%v)", elapsed, hits, err)
}
...
// retry budget. Asserts the bounded-backoff ctx.Done() path in fetchResults.
func TestChaos_PersistentRespectsCtxCancel(t *testing.T) {
	...
	t0 := time.Now()
	_, err := NewClient(srv.URL).Search(ctx, "cancel", 1)
	elapsed := time.Since(t0)
	...
	t.Logf("CHAOS bitsearch ctx-cancel: aborted in %s (err=%v)", elapsed, err)
}
