...
// (Kind stays "native" — this is a compiled-in provider, not a remote jackett
// indexer).
...
// against the default BitSearch base.
...
func (a *ProviderAdapter) DisplayName() string { return "BitSearch" }

// AuthType returns NONE — BitSearch search is anonymous.
...
// Capabilities: BitSearch yields search rows with magnet links (no .torrent
// files), so this provider honestly declares SEARCH + MAGNET_LINK only (§6.E).
...
// Browse is unsupported (BitSearch has no category-browse endpoint).
...
// GetTorrent is unsupported — BitSearch serves magnet links, not .torrent files.
...
// HealthCheck probes BitSearch with a fixed query.
func (a *ProviderAdapter) HealthCheck(ctx context.Context) (*provider.HealthStatus, error) {
	if err := a.client.Health(ctx); err != nil {
		...
		// here would double-report every probe failure (same rationale as the
		// native adapters).
