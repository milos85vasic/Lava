...
func (a *ProviderAdapter) DisplayName() string { return "Nyaa" }

// AuthType returns NONE — Nyaa search is anonymous.
...
// Capabilities: Nyaa's RSS yields search rows with magnet links (via info_hash;
// no .torrent fetch wired), so this provider honestly declares SEARCH +
// MAGNET_LINK only (§6.E).
...
// HealthCheck probes Nyaa with a fixed query.
