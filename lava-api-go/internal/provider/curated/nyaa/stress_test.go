...
// §11.4.85 STRESS + CHAOS coverage for the bounded-retry logic added to the nyaa
// curated RSS provider (maxAttempts=3, 500ms backoff, transient =
// network/timeout-or-5xx, terminal never retried — see fetchFeed / fetchFeedOnce
// in client.go).
//
...
// stressFeed is a Nyaa RSS feed carrying exactly one item with a valid 40-hex
// nyaa:infoHash so a 200 yields exactly one downloadable SearchItem.
...
<nyaa:infoHash>8e1e7ad6a7198d1bea2d8564f40ec3480c490301</nyaa:infoHash>
<nyaa:seeders>100</nyaa:seeders>
<nyaa:leechers>7</nyaa:leechers>
<nyaa:size>400.0 MiB</nyaa:size>
...
func TestStress_ConcurrentFlakyServer(t *testing.T) {
	...
	t.Logf("STRESS nyaa: concurrency=%d ok=%d server_hits=%d server_503s=%d p50=%s p95=%s goroutines before=%d after=%d",
...
func TestStress_LatencyPercentiles(t *testing.T) {
	...
	t.Logf("STRESS nyaa latency: iters=%d p50=%s p95=%s", iters, percentile(lats, 0.50), p95)
}
...
func TestChaos_TransientRecovers(t *testing.T) {
	...
	t.Logf("CHAOS nyaa transient: %d/%d calls recovered from a first-hit 503", okCount, concurrency)
}
...
func TestChaos_PersistentFailsCleanly(t *testing.T) {
	...
	t.Logf("CHAOS nyaa persistent: clean failure in %s after %d attempts (err=%v)", elapsed, hits, err)
}
...
func TestChaos_PersistentRespectsCtxCancel(t *testing.T) {
	...
	t.Logf("CHAOS nyaa ctx-cancel: aborted in %s (err=%v)", elapsed, err)
}
