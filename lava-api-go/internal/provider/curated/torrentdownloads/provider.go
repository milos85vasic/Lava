...
const providerID = "torrentdownloads"

// canonicalSite is shown in the catalogue's baseUrls for informational display.
const canonicalSite = "https://www.torrentdownloads.pro"
...
// against the default TorrentDownloads base.
...
func (a *ProviderAdapter) DisplayName() string { return "TorrentDownloads" }

// AuthType returns NONE — TorrentDownloads search is anonymous.
func (a *ProviderAdapter) AuthType() provider.AuthType { return provider.AuthNone }

// Encoding returns the upstream character set.
func (a *ProviderAdapter) Encoding() string { return "ISO-8859-1" }
...
// Capabilities: the RSS yields search rows with magnet links (built from
// info_hash), so this provider honestly declares SEARCH + MAGNET_LINK only (§6.E).
...
// HealthCheck probes the feed with a fixed query.
