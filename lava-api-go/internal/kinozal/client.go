// Package kinozal is the Lava-domain kinozal.tv scraper.
//
// It wraps an HTTP client with a circuit breaker, handles windows-1251
// transcoding, and exposes typed helpers for each route.
package kinozal
...
var ErrCircuitOpen = errors.New("kinozal: circuit breaker OPEN")

// ErrUnauthorized is returned on auth failures.
var ErrUnauthorized = errors.New("kinozal: unauthorized")

// ErrNoData is returned when the upstream response carries neither a token
// nor a recognisable error marker.
var ErrNoData = errors.New("kinozal: no data")

// Client is the kinozal.tv HTTP client wrapped with a circuit breaker.
type Client struct {
	base    string
	http    *http.Client
...
// NewClient returns a Client whose breaker trips after 5 consecutive failures
// and stays open for 10 s before probing half-open.
...
// Fetch performs a GET against base+path with the given cookie.
...
func (c *Client) PostForm(ctx context.Context, path string, form url.Values, cookie string) ([]byte, int, http.Header, error) {
	var body []byte
	var status int
	var headers http.Header
	encoded := form.Encode()
