...
// on the kinozal.tv client (maxAttempts=3, 500ms backoff, transient =
...
//
// Anti-Bluff (§6.J): the SUT is the REAL kinozal.Client (NewClient) hitting a
...
// breaker state (Closed ⇒ kinozal stays usable; Open ⇒ user is locked out for
...
func flakyUpstream(t *testing.T, failEvery int64) (*httptest.Server, *int64, *int64) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if failEvery > 0 && n%failEvery == 0 {
			...
			http.Error(w, "kinozal flaky", http.StatusServiceUnavailable)
			return
		}
...
func TestStress_ConcurrentFlakyUpstream(t *testing.T) {
	...
	// One shared client (matches production: one breaker fields all kinozal
	...
	for i := 0; i < concurrency; i++ {
		...
		go func() {
			...
			<-start
			t0 := time.Now()
			body, status, err := c.Fetch(ctx, "/browse.php", "")
	...
	// Closed (kinozal stays usable) with 0 consecutive failures.
	...
	t.Logf("STRESS kinozal: concurrency=%d ok=%d upstream_hits=%d upstream_503s=%d breaker=%s/%d p50=%s p95=%s goroutines before=%d after=%d",
...
func TestChaos_TransientKeepsBreakerClosed(t *testing.T) {
	...
	t.Logf("CHAOS kinozal transient: %d/%d calls recovered with breaker Closed and 0 failures", okCount, concurrency)
}
...
func TestChaos_PersistentFailsCleanly(t *testing.T) {
	...
	t0 := time.Now()
	_, status, err := c.Fetch(ctx, "/browse.php", "")
	elapsed := time.Since(t0)
	...
	t.Logf("CHAOS kinozal persistent: clean failure in %s after %d attempts (breaker=%s/%d err=%v)",
...
// is the user-visible "kinozal temporarily unavailable" outcome. Once OPEN,
...
func TestChaos_PersistentTripsBreakerOpen(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		_, _, err := c.Fetch(ctx, "/browse.php", "")
		if err == nil {
	...
	t.Logf("CHAOS kinozal breaker-trip: OPEN after 5 failures, then fast-fails in %s with no upstream hit (err=%v)", elapsed, err)
}
