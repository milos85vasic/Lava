...
See root `/CLAUDE.md` §6.AA. Two-stage distribute (stage 1: debug/dev only → operator verification → stage 2: release/prod only) MANDATORY when an artifact has both variants. For lava-api-go: dev compose container = stage 1; prod compose container OR production registry push = stage 2 ONLY AFTER stage-1 verification. §6.AA-debt is open: mechanical enforcement via `firebase-distribute.sh` default flip + paired `last-version-{debug,release}` per-channel pre-push check is documented but not yet enforced. This submodule MAY add stricter rules but MUST NOT relax.

## §6.Z — Anti-Bluff Distribute Guard (inherited 2026-05-14, per §6.F)

See root `/CLAUDE.md` §6.Z. No artifact may be distributed UNLESS the corresponding end-to-end tests have been **EXECUTED — not source-compiled, EXECUTED** — against the EXACT artifact about to be distributed AND have **passed**. For lava-api-go: every test under `tests/contract/`, `tests/e2e/`, `tests/parity/` MUST execute against the actual binary about to be released, with real Postgres in podman. Pre-distribute test-evidence file required at `.lava-ci-evidence/distribute-changelog/<channel>/<version>-<code>-test-evidence.{md,json}` with matching commit SHA, timestamp within 24h, `BUILD SUCCESSFUL`/`ok` verbatim. Cold-start verification load-bearing canary. §6.Z-debt is open. Forensic anchor: 1.2.19-1039 distribute on Galaxy S23 Ultra (`painterResource()` rejected `<layer-list>`) — agent skipped Compose UI test execution citing the wrong §6.X caveat. Operator's 26th §6.L invocation: "Anti-bluff policy MUST BE ENFORCED ALWAYS!!!" This submodule MAY add stricter rules but MUST NOT relax.


## §11.4.78 — CodeGraph code-intelligence mandate (cascaded from constitution submodule)

Inherited from `constitution/Constitution.md` §11.4.78. Every project worked on by AI coding agents — and `lava-api-go` as a distributable artifact developed alongside the Lava client — MUST install, initialize, and use **CodeGraph** (`https://github.com/colbymchenry/codegraph`, npm package `@colbymchenry/codegraph`): a local SQLite semantic code-knowledge-graph exposed to AI agents over the Model Context Protocol (MCP), 100% local with no cloud or external API. Install globally via npm (no `sudo` — the npm prefix MUST be user-writable). Run `codegraph init` + `codegraph index`: `.codegraph/config.json` is tracked; `.codegraph/codegraph.db` is gitignored with `codegraph index` as its §11.4.77 regeneration mechanism; the `config.json` `exclude` list MUST exclude other-owned submodules and — non-negotiably — every §11.4.10 credential/secret path. Wire the `codegraph serve --mcp` MCP server into every CLI agent the developers use (Claude Code, OpenCode, Qwen Code, Crush, Kimi CLI); configs reference the bare `codegraph` command on `PATH`. Cover the integration with an anti-bluff verification suite whose per-agent end-to-end layer uses an unforgeable challenge (a fact obtainable only by calling a CodeGraph MCP tool); un-runnable agents are documented SKIP gaps per §11.4.3, never faked PASSes. The Lava parent repository carries the wired suite at `scripts/verify-codegraph.sh` + `tests/codegraph/` and the documentation at `docs/CODEGRAPH.md`.

**Cascade requirement:** this anchor (verbatim or by §11.4.78 ID reference) MUST appear in every owned submodule's and `lava-api-go`'s `CONSTITUTION.md`, `CLAUDE.md`, `AGENTS.md`, and `QWEN.md`. See the constitution submodule `Constitution.md` §11.4.78 for the full mandate. Non-compliance is a process violation.
