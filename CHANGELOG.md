...
- **Two real governance gate-integrity bugs found and fixed, via independent reviews dispatched to the `kimi` and `opencode` CLIs in parallel (genuinely separate agents, not Claude subagents) and independently re-verified.** `scripts/check-constitution.sh`'s new third-party-submodule exemption used a plain substring match that would have silently exempted a future submodule merely *starting with* the intended name (e.g. `submodules/superspec-extra`) — fixed to an exact path-component match, same fix applied to the pre-existing `is_helix_dev_owned()` that had the identical latent issue. A second, independent finding: the §6.N propagation-target floor compared an inflated "actual" count (included externally-owned submodules' docs) against a deflated "expected" count (excluded them) — silently tolerating more missing own-org governance docs than intended. Both fixes carry real falsifiability rehearsals (a working exploit reproduced, then confirmed closed).
- **A Robolectric 4.12.2 / host JDK incompatibility fixed across 4 test modules** (`feature/provider_config`, `feature/login`, `feature/credentials`, `core/tracker/client`) — decompiled Robolectric's own native-runtime loader bytecode to confirm it marks itself "loaded" before registration actually completes, explaining a clean (non-wrapped) `UnsatisfiedLinkError`; fixed via `sqliteMode=LEGACY`. 180 tests, 0 failures, 0 regressions.
- **`submodules/superspec` (a spec-kit extension, third-party — `github.com/WangX0111/superspec`) consolidated** onto the project's standard `submodules/` path, eliminating a duplicate registration; the `kimi`/`opencode` speckit integrations (previously dangling, tracked-symlinks-into-gitignored-state) repaired and rescaffolded as portable, checked-out files.
- **A background governance audit (BG-002) confirmed zero genuine gaps** in this project's existing anti-bluff/all-test-types/zero-defect governance coverage — the operator's concerns were already covered, several clauses near-verbatim, by the existing HelixConstitution corpus; one honest compliance-backlog finding recorded (constitution's 7 nested engines are pinned but not yet invoked by any Lava script) with no new rule needed since the existing rules already forbid that failure mode.
- **`lava-api-go`'s existing systemd `--user` deployment (already shipped 2026-08-12, not new this cycle) was live-verified for real this cycle**: installed, `curl`'d against the host's actual LAN IP (not loopback) returning `200`, `systemctl --user stop`/`start` proven to genuinely control the container lifecycle (not coincidental prior state), and `loginctl` linger confirmed enabled so the service survives reboot without an active login session.
- **§6.Y:** Android versionCode 1086→1087 (versionName HELD at 1.3.17 — no `:app` module source changed this cycle, only build/test-tooling + lava-api-go + governance scripts); lava-api-go Name 2.3.34→2.3.35, Code 2334→2335 (real user-facing operational fix, patch bump per §6.Y clause 3).
